...
fzf-history-widget() {
  ...
  # The `/` inside that bracket expression MUST stay backslash-escaped: macOS awk
  # (BWK awk 20200816, /usr/bin/awk) ends the /.../ regex literal at the first bare `/`
  # even inside [...], and dies with "nonterminated character class". gawk tolerates it,
  # so an unescaped slash works on Linux and silently breaks Ctrl-R on macOS.
